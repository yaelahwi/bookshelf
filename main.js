...
const id_element = document.createElement("h1");
id_element.setAttribute("id", "id");
...
container.append(id_element, title_element, author_element, year_element);
...
if (isComplete) {
    ...
    btn_edit.innerHTML = '<p><i class="fa-solid fa-edit"></i>edit</p>';
    btn_edit.classList.add("edit");
    btn_edit.addEventListener("click", function() {
        handler_edit_book(id);
    });

    const btn_red = document.createElement("button");
    btn_red.innerHTML = '<p><i class="fa-regular fa-trash-can"></i></p>';
    ...
    container.append(btn_green, btn_edit, btn_red);
} else {
    ...
    container.append(btn_finish, btn_edit, btn_red); 
    }
    return container;
}
...
function handler_edit_book(id){
    isEditing = true;
    bookContent = find_book(id)
    bookTemp = find_book(id)
    const book = find_book(id);
    if (book !== null) {
        document.getElementById('inputBookTitle').value = bookContent.title;
        document.getElementById('inputBookAuthor').value = bookContent.author;
        document.getElementById('inputBookYear').value = bookContent.year;
        document.getElementById('inputBookIsComplete').checked = bookContent.isComplete;
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    submitForm.addEventListener("submit", (e) => {
        e.preventDefault();
        isEditing ? updateBook() : add_book()
        resetValue()
        swal("Berhasil menambahkan/merubah buku!", {
			icon: "success",
		});
    });
